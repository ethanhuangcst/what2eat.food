...
export default function SetPasswordPageClient() {
  ...
  usePageTitle("eat.set_password.title");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (password !== confirm) {
      setErrorKey("eat.errors.password_mismatch");
      return;
    }
    try {
      ...
      if (resolved === "eat.errors.token_expired" || key === "errors.token_expired") {
    ...
    <PublicShell>
      <main id="content" className="auth-main">
        ...
        <div data-set-done hidden={!done}>
          <h1>{t("eat.set_password.done_title")}</h1>
          <p className="lead">{t("eat.set_password.done_lead")}</p>
          <Link className="btn" href="/login">
            {t("eat.set_password.sign_in")}
          </Link>
        </div>

        <div data-set-form hidden={done}>
          <h1>{t("eat.set_password.title")}</h1>
          {!modeReset ? (
            <p className="lead" data-set-empty-lead>
              {t("eat.set_password.lead")}
            </p>
          ) : null}
          {modeReset ? (
            <p className="lead" data-set-reset-lead>
              {t("eat.set_password.reset_lead")}
            </p>
          ) : null}

          {sessionExpired ? (
            <div className="callout is-warn" data-set-error-session>
              <h2>{t("eat.errors.reset_link_expired_title")}</h2>
              <p>{t("eat.errors.reset_link_expired_body")}</p>
              <p>
                <Link className="btn" href="/reset-password">
                  {t("eat.errors.reset_link_expired_action")}
                </Link>
              </p>
            </div>
          ) : null}
