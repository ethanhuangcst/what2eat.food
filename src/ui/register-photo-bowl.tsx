...
export function RegisterPhotoBowl({ photoUrl, onPhotoChange, onPhotoError }: Props) {
  ...
  const [fileName, setFileName] = useState<string | null>(photoUrl ? t("eat.register.photo_choose") : null);

  useEffect(() => {
    if (photoUrl != null) {
      setPreview(photoUrl);
      setFileName(t("eat.register.photo_choose"));
    }
  }, [photoUrl]);
  ...
  function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > PHOTO_MAX_BYTES) {
      onPhotoError?.("eat.errors.photo_too_large");
      e.target.value = "";
      return;
    }
      ...
      >
        {!hasPhoto ? (
          <>
            ...
            <span className="register-photo__cta">{t("eat.register.photo_add")}</span>
          </>
        ) : (
          <img className="register-photo__img" src={preview!} alt="" />
        )}
      </button>
      <p className="register-photo__name">{fileName ?? t("eat.register.photo_none")}</p>
    </div>
  );
}
