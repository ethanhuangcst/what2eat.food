...
describe("validateRegisterClient", () => {
  ...
  it("should_flag_age_out_of_range_when_provided", () => {
    ...
    expect(errors.age).toBe("eat.errors.age_out_of_range");
  });

  it("should_flag_password_too_short", () => {
    const errors = validateRegisterClient({ ...valid, password: "abc", confirmPassword: "abc" });
    expect(errors.password).toBe("eat.errors.password_too_short");
    expect(errors.password_confirm).toBe("eat.errors.password_too_short");
  });
  ...
  it("should_flag_password_mismatch", () => {
    ...
    expect(errors.password_confirm).toBe("eat.errors.password_mismatch");
  });

  it("should_flag_name_required", () => {
    expect(validateRegisterClient({ ...valid, name: "" }).name).toBe("eat.errors.name_required");
  });
  ...
  it("should_map_api_errors_to_fields", () => {
    ...
    expect(mapApiErrorToField("errors.unknown").errorKey).toBe("eat.errors.unknown");
  });

  it("should_map_csrf_to_form_level", () => {
    const mapped = mapApiErrorToField("errors.csrf");
    expect(mapped.formLevel).toBe(true);
    expect(mapped.errorKey).toBe("eat.errors.csrf");
  });
});
...
describe("firstRegisterField", () => {
  it("should_return_first_invalid_field_in_order", () => {
    expect(
      firstRegisterField({
        password: "eat.errors.password_too_short",
        name: "eat.errors.name_required",
      }),
    ).toBe("name");
  });
});
