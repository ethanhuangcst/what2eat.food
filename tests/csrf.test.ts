...
describe("csrfOk", () => {
  it("should_accept_matching_origin", () => {
    const request = new Request("http://localhost:3020/api/auth/login", {
      method: "POST",
      headers: { origin: "http://localhost:3020", host: "localhost:3020" },
    });
  ...
  it("should_reject_cross_origin", () => {
    const request = new Request("http://localhost:3020/api/auth/login", {
      method: "POST",
      headers: { origin: "http://evil.example", host: "localhost:3020" },
    });
  ...
  it("should_reject_invalid_referer", () => {
    const request = new Request("http://localhost:3020/api/auth/login", {
      method: "POST",
      headers: { referer: "http://evil.example/login", host: "localhost:3020" },
    });
    expect(csrfOk(request)).toBe(false);
  });
  ...
  it("should_reject_invalid_referer_url", () => {
    const request = new Request("http://localhost:3020/api/auth/login", {
      method: "POST",
      headers: { referer: "not a url", host: "localhost:3020" },
    });
    expect(csrfOk(request)).toBe(false);
  });
  ...
  it("should_accept_lan_dev_origin_when_configured", () => {
    ...
    const request = new Request("http://192.168.1.10:3020/api/auth/login", {
      method: "POST",
      headers: { origin: "http://192.168.1.10:3020", host: "192.168.1.10:3020" },
    });
    expect(csrfOk(request)).toBe(true);
    vi.unstubAllEnvs();
  });
